    ...
    <AnimatePresence>
      <div className={styles.container} >
        {calendarMonth.map((day, index) => {
          return <motion.button initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ ease: 'easeInOut', duration: 0.3 }} disabled={day === ''} onClick={() => selectDay(Number(day))} className={highlightActiveDate(Number(day)) ? styles.activeDate : styles.dates} key={`${day}-${index}`}>{day}</motion.button>
        })
